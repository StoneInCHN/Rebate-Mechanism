...
public class EndUserController extends MobileBaseController {
	...
	public @ResponseBody ResponseOne<Map<String, Object>> login(
		...
		if (StringUtils.isEmpty(cellPhoneNum) || !isMobileNumber(cellPhoneNum)) {
			...
			response.setDesc(Message.error("rebate.mobile.invaliable")
					.getContent());
			return response;
		}
		if (LogUtil.isDebugEnabled(EndUserController.class)) {
			LogUtil.debug(EndUserController.class, "login",
					"Fetching User from database with CellPhoneNum: %s",
					cellPhoneNum);
		}
		...
		if (!StringUtils.isEmpty(password)) {
			if (loginUser == null) {
				response.setCode(CommonAttributes.FAIL_LOGIN);
				response.setDesc(Message.error("rebate.user.noexist")
						.getContent());
				return response;
			}
			try {
				password = KeyGenerator.decrypt(password,
			...
			if (password.length() < setting.getPasswordMinlength()) {
				response.setCode(CommonAttributes.FAIL_LOGIN);
				response.setDesc(Message.error("rebate.pwd.length.error",
						setting.getPasswordMinlength()).getContent());
				return response;
			}

			if (!DigestUtils.md5Hex(password).equals(loginUser.getLoginPwd())) {
				response.setCode(CommonAttributes.FAIL_LOGIN);
				response.setDesc(Message.error("rebate.nameorpwd.error")
						.getContent());
				return response;
			}
		}
		 ...
		 */
			...
			SMSVerificationCode smsVerficationCode = endUserService
					.getSmsCode(cellPhoneNum);
			if (smsVerficationCode == null) {
				response.setCode(CommonAttributes.FAIL_LOGIN);
				response.setDesc(Message.error("rebate.sms.invaliable")
						.getContent());
				return response;
			} else {
				...
				if (timeoutToken != null
						&& !TokenGenerator.smsCodeTokenTimeOut(timeoutToken,
								setting.getSmsCodeTimeOut())) {
					if (!smsCode.equals(code)) {
						response.setCode(CommonAttributes.FAIL_LOGIN);
						response.setDesc(Message
								.error("rebate.sms.token.error").getContent());
						return response;
					} else {
						endUserService.deleteSmsCode(cellPhoneNum);
						if (loginUser == null) {// 用户不存在，登录成功的同时注册该手机号
							loginUser = endUserService.userReg(cellPhoneNum,
									null, null);
							if (LogUtil.isDebugEnabled(EndUserController.class)) {
								LogUtil.debug(
					...
					response.setCode(CommonAttributes.FAIL_LOGIN);
					response.setDesc(Message.error("rebate.sms.token.timeout")
							.getContent());
					return response;
				}
			}
		...
		if (AccountStatus.LOCKED.equals(loginUser.getAccountStatus())
			...
			response.setCode(CommonAttributes.FAIL_LOGIN);
			response.setDesc(Message.error("rebate.current.user.invalid")
					.getContent());
			return response;
		}

		String[] properties = { "id", "cellPhoneNum", "nickName", "userPhoto",
				"recommender", "agent.agencyLevel", "area.id", "area.name",
				"curScore", "curLeMind", "curLeScore", "totalScore",
				"totalLeMind", "totalLeScore", "curLeBean", "totalLeBean",
				"isBindWeChat" };
		Map<String, Object> map = FieldFilterUtils.filterEntityMap(properties,
				loginUser);
	...
	public @ResponseBody BaseResponse resetPwd(HttpServletRequest request,
		...
		if (StringUtils.isEmpty(password)
			...
			response.setCode(CommonAttributes.FAIL_RESET_PWD);
			response.setDesc(Message.error("rebate.pwd.null.error")
					.getContent());
			return response;
		}
		...
		if (smsVerficationCode == null) {
			...
			if (timeoutToken != null
				...
				if (!smsCode.equals(code)) {
					response.setCode(CommonAttributes.FAIL_RESET_PWD);
					response.setDesc(Message.error("rebate.sms.token.error")
							.getContent());
					return response;
				} else {
					endUserService.deleteSmsCode(cellPhoneNum);
	...
	public @ResponseBody BaseResponse getSmsCode(HttpServletRequest request,
		...
		if (StringUtils.isEmpty(cellPhoneNum) || !isMobileNumber(cellPhoneNum)) {
			...
			if (smsCodeType.equals(SmsCodeType.REG)) {// 注册（user应不存在）
				if (endUser != null) {
					response.setCode(CommonAttributes.FAIL_SMSTOKEN);
					response.setDesc(Message.error("rebate.mobile.used")
							.getContent());
					return response;
				}
			...
			SMSVerificationCode smsVerificationCode = endUserService
					.getSmsCode(cellPhoneNum);
			...
			ToolsUtils.sendSmsMsg(cellPhoneNum, setting.getSmsContentTemp()
					+ smsCode.toString() + setting.getSmsPostfix());// 发送短信验证码
			...
			newSmsCode.setTimeoutToken(new Long(new Date().getTime())
					.toString());
			...
			if (LogUtil.isDebugEnabled(EndUserController.class)) {
				LogUtil.debug(
						EndUserController.class,
						"getSmsCode",
						"send SmsCode for cellPhone number: %s,smsCode: %s,type: %s",
						cellPhoneNum, smsCode.toString(),
						smsCodeType.toString());
			}
			response.setCode(CommonAttributes.SUCCESS);
	...
	public @ResponseBody ResponseOne<Map<String, Object>> reg(
			@RequestBody UserRequest userReg) {
		ResponseOne<Map<String, Object>> response = new ResponseOne<Map<String, Object>>();
		String serverPrivateKey = setting.getServerPrivateKey();
		...
		if (!StringUtils.isEmpty(recommenderMobile)) {
			EndUser recommender = endUserService
					.findByUserMobile(recommenderMobile);
			if (recommender == null) {
				response.setCode(CommonAttributes.FAIL_REG);
				response.setDesc(Message.error("rebate.recommender.no.exist")
						.getContent());
				return response;
			}
		}
		...
		try {
			password = KeyGenerator.decrypt(password,
					RSAHelper.getPrivateKey(serverPrivateKey));
			password_confirm = KeyGenerator.decrypt(password_confirm,
					RSAHelper.getPrivateKey(serverPrivateKey));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		EndUser regUser = endUserService.userReg(cellPhoneNum, password,
				recommenderMobile);
		if (LogUtil.isDebugEnabled(EndUserController.class)) {
			LogUtil.debug(
					EndUserController.class,
					"Reg",
					"EndUser Reg. User CellPhoneNum: %s, Recommender Mobile: %s",
					cellPhoneNum, recommenderMobile);
		}
	...
	public @ResponseBody ResponseOne<Map<String, Object>> editUserInfo(
		...
		if (LogUtil.isDebugEnabled(EndUserController.class)) {
			LogUtil.debug(EndUserController.class, "edit user info",
					"Edit EndUser Info. NickName: %s, area: %s", endUser
							.getNickName(), endUser.getArea() != null ? endUser
	...
	public @ResponseBody BaseResponse updatePwd(HttpServletRequest request,
		...
		if (LogUtil.isDebugEnabled(EndUserController.class)) {
			LogUtil.debug(EndUserController.class, "Update Password",
					"EndUser Update Password. CellPhone: %s,Type: %s",
					cellPhoneNum, smsCodeType.toString());
		}
	...
	public @ResponseBody ResponseOne<Map<String, Object>> editUserPhoto(
			UserRequest req) {
		ResponseOne<Map<String, Object>> response = new ResponseOne<Map<String, Object>>();
		...
		if (LogUtil.isDebugEnabled(EndUserController.class)) {
			LogUtil.debug(EndUserController.class, "editUserPhoto",
					"Edit photo for EndUser. CellPhoneNum: %s, PhotoUrl: %s",
					endUser.getCellPhoneNum(), endUser.getUserPhoto());
		}
	...
	@UserValidCheck
	public @ResponseBody ResponseOne<Map<String, Object>> getUserInfo(
	...
	public @ResponseBody ResponseOne<Map<String, Object>> getQrCode(
			@RequestBody BaseRequest req) {
		ResponseOne<Map<String, Object>> response = new ResponseOne<Map<String, Object>>();
		Long userId = req.getUserId();
		String token = req.getToken();
		...
		if (endUser.getQrImage() == null) {
			String content = "{\"flag\":\"" + DigestUtils.md5Hex("翼享生活")
					+ "\",\"cellPhoneNum\":\"" + endUser.getCellPhoneNum()
					+ "\"}";
	...
	public @ResponseBody ResponseMultiple<Map<String, Object>> getRecommendRec(
		...
		Page<UserRecommendRelation> page = userRecommendRelationService
				.getRelationsByRecommender(userId, pageable);
		String[] propertys = { "id", "endUser.nickName",
				"endUser.totalLeScore", "endUser.userPhoto" };
	...
	public @ResponseBody ResponseMultiple<Map<String, Object>> scoreRec(
		...
		String[] propertys = { "id", "seller.name", "createDate",
				"seller.storePictureUrl", "rebateScore", "paymentType",
				"userCurScore" };
		List<Map<String, Object>> result = FieldFilterUtils
				.filterCollectionMap(propertys, page.getContent());
	...
	public @ResponseBody ResponseMultiple<Map<String, Object>> leMindRec(
		...
		String[] propertys = { "id", "amount", "createDate", "score",
				"userCurLeMind" };
		List<Map<String, Object>> result = FieldFilterUtils
				.filterCollectionMap(propertys, page.getContent());
	...
	public @ResponseBody ResponseMultiple<Map<String, Object>> leScoreRec(
		...
		String[] propertys = { "id", "seller.name", "createDate",
				"seller.storePictureUrl", "amount", "leScoreType",
				"userCurLeScore", "recommender", "recommenderPhoto",
				"withdrawStatus" };
		List<Map<String, Object>> result = FieldFilterUtils
				.filterCollectionMap(propertys, page.getContent());
	...
	public @ResponseBody ResponseMultiple<Map<String, Object>> leBeanRec(
		...
		String[] propertys = { "id", "amount", "createDate", "userCurLeBean",
				"type" };
		List<Map<String, Object>> result = FieldFilterUtils
				.filterCollectionMap(propertys, page.getContent());
	...
	public @ResponseBody BaseResponse favoriteSeller(
		...
		if (endUser == null || seller == null) {
			...
			response.setDesc(Message.error("rebate.request.param.invalid")
					.getContent());
			return response;
		}
		...
		if (request.getIsFavoriteAdd()) {
			if (!sellers.contains(seller)) {
				...
				int favoriteNum = seller.getFavoriteNum() == null ? 0 : seller
						.getFavoriteNum();
	...
	public @ResponseBody ResponseMultiple<Map<String, Object>> favoriteSellerList(
		...
		String[] propertys = { "id", "name","sellerCategory.categoryName" ,"latitude", "longitude",
				"rateScore","avgPrice" };
		List<Map<String, Object>> result = FieldFilterUtils
				.filterCollectionMap(propertys, page.getContent());
