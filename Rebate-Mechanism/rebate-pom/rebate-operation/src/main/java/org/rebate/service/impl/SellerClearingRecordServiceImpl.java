...
public class SellerClearingRecordServiceImpl extends BaseServiceImpl<SellerClearingRecord, Long>
    implements SellerClearingRecordService {
   ...
   */
  @Override
  public List<SellerClearingOrders> getNeedClearingRecords(Date startDate, Date endDate,
      PaymentChannel channel) {

    List<SellerClearingOrders> clearingOrdersList = new ArrayList<SellerClearingOrders>();// 货款记录列表

    Map<Long, BigDecimal> sellerAmountMap = new HashMap<Long, BigDecimal>();// 商家Id(KEY) : 订单金额
                                                                            // (VALUE)
    Map<Long, BigDecimal> sellerIncomeMap = new HashMap<Long, BigDecimal>();// 商家Id(KEY) : 结算金额
                                                                            // (VALUE)
    Map<Long, Set<Order>> sellerOrdersMap = new HashMap<Long, Set<Order>>();// 商家Id(KEY) : 订单列表
                                                                            // (VALUE)
    ...
    LogUtil.debug(this.getClass(), "getNeedClearingRecords", "总共订单个数: %s, 实际需要结算的订单个数: %s",
        orderList.size(), orders.size());
    ...
    if (!sellerAmountMap.isEmpty() && !sellerIncomeMap.isEmpty() && !sellerOrdersMap.isEmpty()) {
      ...
      for (Map.Entry<Long, BigDecimal> entry : sellerAmountMap.entrySet()) {
        ...
        SystemConfig clearingMinLimit =
            systemConfigService.getConfigByKey(SystemConfigKey.CLEARING_MINIMUM_LIMIT);// 商家货款结算最低金额限制
        if (clearingMinLimit != null && clearingMinLimit.getConfigValue() != null) {
          ...
          if (record.getAmount().compareTo(minLimit) == -1) {// 货款金额 小于 最低限制
            LogUtil.debug(this.getClass(), "getNeedClearingRecords",
                "商家ID:%s, 货款金额 :%s, 不足最低结算金额 %s, 等下次凑够了金额再结算", seller.getId(), record.getAmount()
                    .toString(), clearingMinLimit.getConfigValue());
        ...
        if (defaultCard != null) {
          ...
          LogUtil.debug(this.getClass(), "getNeedClearingRecords", "未能找到商家用户默认银行卡,商家用户Id: %s",
              endUser != null ? endUser.getId() : "");
          ...
          LogUtil
              .debug(this.getClass(), "getNeedClearingRecords",
                  "商家用户 %s 未添加默认银行卡,待添加银行卡后执行单笔货款提现!", endUser != null ? endUser.getCellPhoneNum()
                      : "");
  ...
  public String sellerClearingByAllinPay(List<SellerClearingOrders> clearingOrdersList) {
    ...
    try {
      // 总代付的金额(单位 分， 字符串)
      String totalPayStr =
          totalPay.multiply(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_HALF_UP).toString();// 金额单位：分

      // 开始批量代付
      List<SellerClearingRecord> recordList =
          tranxService.batchDaiFu(false, totalItem, totalPayStr, records);
      ...
      e.printStackTrace();
      LogUtil.debug(this.getClass(), "sellerClearingByAllinPay", "商家货款批量代付(通联渠道)失败  捕获异常: %s",
          e.getMessage());
  ...
  public String sellerClearingByJiuPai(List<SellerClearingOrders> clearingOrdersList) {
    if (clearingOrdersList.size() > 0) {
      ...
      try {
        ...
        if (jsonArray.size() > 0) {
          ...
          for (int j = 0; j < jsonArray.size(); j++) {
            ...
            LogUtil
                .debug(
                    this.getClass(),
                    "sellerClearingByJiuPai",
                    "merchantId: %s, batchNo: %s, jrnNo: %s, mercOrdNo: %s,errorCode: %s, errorMsg: %s",
                    merchantId, batchNo, jrnNo, mercOrdNo, errorCode, errorMsg);
            if (!mercOrdNo.startsWith(merchantId) || !jrnNo.startsWith(batchNo)) {
              LogUtil.debug(this.getClass(), "sellerClearingByJiuPai",
                  "merchantId + withDrawSn != 商户订单号(mercOrdNo) 或   batchNo + sn != 交易流水 (jrnNo)");
              continue;
            }
        ...
        e.printStackTrace();
        LogUtil.debug(this.getClass(), "sellerClearingByJiuPai", "商家货款批量代付(九派渠道)失败  捕获异常: %s",
            e.getMessage());
        return null;
      }
      return reqSn;
  ...
  public Message singlePayByAllinpay(SellerClearingRecord record, BankCard bankCard) {
    try {
      ...
      Map<String, String> resultMap =
          tranxService.singleDaiFushi(false, bankCard.getOwnerName(), bankCard.getCardNum(),
              amountPenny.toString());
      if (resultMap.containsKey("status") && resultMap.containsKey("req_sn")) {
        ...
        LogUtil.debug(this.getClass(), "singlePayByAllinpay",
            "(通联渠道)单笔实时付款返回  status:%s, req_sn:%s, err_msg:%s", status, req_sn, err_msg);

        if ("success".equals(status) || "error".equals(status)) {// 最终结果
          singlePayHandle(record, status, req_sn, newClearingSn, err_msg, handlingCharge,
              PaymentChannel.ALLINPAY);
          return Message.success("rebate.message.success");
        }
        if ("wait".equals(status)) {// 中间状态，还需要等待查询
          singlePayWait(record, status, req_sn, newClearingSn, err_msg, handlingCharge,
              PaymentChannel.ALLINPAY, null, null);
  ...
  public Message singlePayByJiuPai(SellerClearingRecord record, BankCard bankCard) {
    ...
    BigDecimal payAmount = record.getAmount().abs().subtract(handlingCharge);
    String amountPenny =
        payAmount.multiply(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_HALF_UP).toString();
    try {
      GateWayService gateWayService = new GateWayService();
      ...
      // //test
        if ("S".equals(orderSts)) {// 处理成功
          singlePayHandle(record, "success", mcSequenceNo, newClearingSn, rspMessage,
              handlingCharge, PaymentChannel.JIUPAI);
          return Message.success(rspMessage);
        }
        if ("F".equals(orderSts) || "R".equals(orderSts)) {// 处理失败
          String errMsg = rspMessage;
          if ("F".equals(orderSts))
            errMsg = "处理失败";
          if ("R".equals(orderSts))
            errMsg = "退汇";
          singlePayHandle(record, "error", mcSequenceNo, newClearingSn, errMsg, handlingCharge,
              PaymentChannel.JIUPAI);
          return Message.error(rspMessage);
        }
        if ("U".equals(orderSts) || "P".equals(orderSts) || "N".equals(orderSts)) {// 处理中
          String errMsg = rspMessage;
          if ("U".equals(orderSts))
            errMsg = "订单初始化";
          if ("P".equals(orderSts))
            errMsg = "处理中";
          if ("N".equals(orderSts))
            errMsg = "待人工处理";
          singlePayWait(record, "wait", mcSequenceNo, newClearingSn, errMsg, handlingCharge,
              PaymentChannel.JIUPAI, mcTransDateTime, orderNo);
          return Message.error(rspMessage);
        }
      } else {// 例如rspCode=IPS05000 rspMessage=余额不足
        singlePayHandle(record, "error", mcSequenceNo, newClearingSn, rspMessage, handlingCharge,
            PaymentChannel.JIUPAI);
        return Message.error(rspMessage);
      }
      return Message.error("rebate.common.system.error");
   ...
   */
  @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
  private void singlePayWait(SellerClearingRecord oldRecord, String status, String reqSn,
      String newClearingSn, String errMsg, BigDecimal handlingCharge, PaymentChannel channel,
      String mcTransDateTime, String orderNo) {
   ...
   */
  @Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
  private void singlePayHandle(SellerClearingRecord oldRecord, String status, String req_sn,
      String newClearingSn, String err_msg, BigDecimal handlingCharge, PaymentChannel channel) {
    oldRecord.setValid(false);// 标记旧的货款记录无效
    update(oldRecord);// 更新旧的货款记录
  ...
  public List<ClearingOrderRelation> getRelationListByRecordId(Long recordId) {
    ...
    List<ClearingOrderRelation> relations =
        clearingOrderRelationService.findList(null, filters, null);
    return relations;
  }
  ...
  // }else {
  // SystemConfig feePercentage =
  // systemConfigService.getConfigByKey(SystemConfigKey.TRANSACTION_FEE_PERCENTAGE);
